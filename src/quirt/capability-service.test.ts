...
describe("Quirt capability inventory", () => {
  it("reports the exact native catalog, host facts, validation boundaries, and durable current-host probe", async () => {
    ...
    assert.equal(inventory.protocolVersion, 1); assert.equal(inventory.stateSchemaVersion, QUIRT_STATE_SCHEMA_VERSION); assert.equal(inventory.implementedOperationCount, 148); assert.deepEqual(inventory.operations, QUIRT_OPERATIONS);
    ...
    const record = f.state.native.getObject("quirt_capability_probes", "current-host", owner); assert.equal(record.data.implementedOperationCount, 148);
  });
  ...
  it("distinguishes unavailable host executables from implemented source without claiming provider support", async () => {
    ...
    assert.equal((inventory.host as { bootId: null }).bootId, null); const status = await f.service.status(owner); assert.equal(status.status, "degraded"); assert.equal((status.capabilitySummary as { operationCount: number }).operationCount, 148);
  });
});
